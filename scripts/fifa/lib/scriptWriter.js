...
const SYSTEM_PROMPT = `You are a viral YouTube script writer specializing in FIFA and soccer. It is 2026 and the FIFA World Cup is happening across the USA, Canada, and Mexico (June 11 – July 19). 48 teams, 104 matches, MetLife Stadium hosting the Final.
...
{
  "title": "string (YouTube title, ≤100 chars, include emoji, SEO keywords)",
...
1. HOOK — shocking stat, controversial question, or cliffhanger (first 5 seconds)
2. CONTEXT — why this matters right now
3. DEEP DIVE — the meat (facts, analysis, drama)
4. HOT TAKE — your controversial personal opinion
5. CTA — "Comment below", "Subscribe for daily World Cup coverage"
...
async function generateScript(articles, formatHint = 'auto') {
  ...
  console.log(`[scriptWriter] "${script.title}" — ${script.format}, ${script.scenes.length} scenes, ${script.totalDuration}s`);
  return script;
}
