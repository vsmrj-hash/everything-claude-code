...
async function runPipeline(options = {}) {
  ...
  const t0 = Date.now();
  console.log('\n' + '='.repeat(62));
  console.log('🏆  FIFA Content Creator  |  ' + new Date().toUTCString());
  console.log('='.repeat(62) + '\n');

  try {
    // ── 1. News ─────────────────────────────────────────────────
    console.log('📰  Step 1/6: Scraping FIFA news...');
    ...
    // ── 2. Script ────────────────────────────────────────────────
    console.log('✍️   Step 2/6: Writing video script with Claude...');
    ...
    // ── 3. Media ─────────────────────────────────────────────────
    console.log('🖼️   Step 3/6: Sourcing royalty-free media...');
    ...
    if (config.ELEVENLABS_API_KEY || config.OPENAI_API_KEY) {
      console.log('🎙️   Step 4/6: Generating AI voice narration...');
      ...
      console.log('🎙️   Step 4/6: Skipping narration (no TTS key configured)\n');
    }

    // ── 5. Build video ───────────────────────────────────────────
    console.log('🎬  Step 5/6: Building video with FFmpeg...');
    ...
    if (options.skipUpload) {
      console.log('📤  Step 6/6: Upload skipped (--skip-upload)\n');
    } else {
    ...
    if (uploadResult?.videoUrl) {
      console.log('✅  DONE — Published to YouTube:');
      console.log('    ' + uploadResult.videoUrl);
    } else {
      console.log('✅  DONE — Video saved locally:');
      console.log('    ' + videoPath);
    }
    console.log(`⏱️   Pipeline took ${elapsed}s`);
    console.log('='.repeat(62) + '\n');
    ...
    console.error('    See scripts/fifa/logs/errors.log for the full trace\n');
    throw err;
  }
}
